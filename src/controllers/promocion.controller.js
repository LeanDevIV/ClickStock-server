...
export const crearPromocion = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ mensaje: "Promoción creada correctamente", promocion });
  } catch (error) {
    res
      .status(500)
      .json({ mensaje: "Error al crear la promoción", error: error.message });
  }
};
...
export const obtenerPromocionPorId = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ mensaje: "Error al obtener la promoción", error: error.message });
  }
};

export const actualizarPromocion = async (req, res) => {
  try {
    const promocionActualizada = await actualizarPromocionService(
      req.params.id,
      req.body
    );
